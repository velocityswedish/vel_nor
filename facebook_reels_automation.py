"""
Facebook Reels Automation - Bilingual English/Norwegian Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Norwegian branding
"""
...
# Norwegian translations for display
CATEGORIES_NORWEGIAN = {
    ...
    "Growth": "Vekst",
    "Purpose": "Formål",
    "Mindfulness": "Aksomhet",
}

# Edge TTS voices
ENGLISH_VOICE = "en-US-GuyNeural"
NORWEGIAN_VOICE = "nb-NO-FinnNeural"
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 3} unique {category_english} phrases for English speakers learning Norwegian.
...
2. Norwegian translation (with commas matching the rhythm)
...
IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before. Be creative!"""
            ...
            payload = {
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "You are a Norwegian teacher. Create short, natural phrases with pauses."},
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "norwegian": "Tro på deg selv.", "pronunciation": "Troh poh day sel-va."},
            {"english": "You are capable of amazing things.", "norwegian": "Du er i stand til utrolige ting.", "pronunciation": "Doo er i stand til oot-troo-li-ge ting."},
            {"english": "Dream big, start small.", "norwegian": "Drøm stort, start smått.", "pronunciation": "Druhm stort, start smowt."},
            {"english": "Your future is created by your actions.", "norwegian": "Din fremtid er skapt av dine handlinger.", "pronunciation": "Deen frem-teed er skapt av dee-ne hand-ling-er."},
            {"english": "Never give up on your dreams.", "norwegian": "Aldri gi opp drømmene dine.", "pronunciation": "Al-dree gee oop druhm-ma-nee dee-ne."},
        ],
        "Love": [
            {"english": "Love yourself first.", "norwegian": "Elsk deg selv først.", "pronunciation": "Elsk day sel-va furst."},
            {"english": "Love makes everything possible.", "norwegian": "Kjærlighet gjør alt mulig.", "pronunciation": "Shoar-li-het yur alt moo-lee."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        norwegian_file = output_dir / f"norwegian_{i}.mp3"
        combined_file = output_dir / f"combined_{i}.mp3"

        print(f"\n  Phrase {i+1}:")
        print(f"    EN: {phrase['english']}")
        print(f"    NO: {phrase['norwegian']}")
        ...
        # Generate Norwegian audio
        no_success = asyncio.run(generate_single_audio(phrase["norwegian"], NORWEGIAN_VOICE, str(norwegian_file)))
        if no_success:
            print(f"    ✓ Norwegian: {norwegian_file.name}")
        else:
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "2", str(norwegian_file)]
            subprocess.run(cmd, capture_output=True)

        # Get ACTUAL durations
        en_duration = get_audio_duration(str(english_file))
        no_duration = get_audio_duration(str(norwegian_file))

        # Add pause between English and Norwegian
        pause_between = 0.5
        total_duration = en_duration + pause_between + no_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + NO: {no_duration:.2f}s)")

        # Combine audio files
        cmd = [
            "ffmpeg", "-y",
            "-i", str(english_file),
            "-i", str(norwegian_file),
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{norwegian_file.as_posix()}'\n")
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    # Norwegian text
    norwegian_y = english_y + total_height + 110  # Increased from 100
    norwegian_lines = wrap_text(norwegian, font_large, VIDEO_WIDTH - 140)
    total_height = len(norwegian_lines) * 95  # Increased from 75

    draw.rectangle(
        [(60, norwegian_y - 55), (VIDEO_WIDTH - 60, norwegian_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(norwegian_lines):
        y_pos = norwegian_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = norwegian_y + total_height + 90  # Increased from 80
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_NORWEGIAN[category_english]})")
    print(f"{'='*80}\n")
    ...
    print("\n[3/4] Generating audio (English + Norwegian with 500ms pause)...")
    ...
    print(f"  🏷️  Branding: Velocity Norwegian")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇳🇴 VELOCITY NORWEGIAN - FACEBOOK REELS AUTOMATION 🇳🇴")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ Velocity Norwegian branding at bottom")
