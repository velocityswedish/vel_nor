...
def generate_video_metadata(category: str, num_phrases: int):
    """Generate Norwegian title, description, and tags for the video"""

    title = f"Norsk Læring: {num_phrases} Essensielle {category} Setninger 🇳🇴"

    description = f"""Lær {num_phrases} essensielle norske setninger for {category}!

Perfekt for nybegynnere som lærer norsk. Hver setning inkluderer:
✅ Engelsk setning
✅ Norsk oversettelse
✅ Uttaleguide
✅ Brukskontekst

Abonner for daglige norsktimer!

#NorskLæring #NorskeTimer #NorskForNybeggynere #Språklæring
#Norsk #Utdanning #Tutorial #DagligNorsk #{category.replace(' ', '')}
"""
...
def upload_to_youtube(video_path, title, description, tags=None, category_id='27'):
    ...
    if tags is None:
        tags = ['norsk læring', 'norwegian lessons']
...
def main():
    ...
    # Generate Norwegian metadata for test
    title = "Norsk Læring: 5 Essensielle Motivasjon Setninger 🇳🇴"
    description = "Lær 5 essensielle norske setninger for Motivasjon! #NorskLæring #NorskeTimer"
    tags = ['norsk læring', 'norwegian lessons', 'motivasjon']
